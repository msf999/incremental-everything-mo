...
async function onActivate(plugin: ReactRNPlugin) {
  ...
  await plugin.app.registerPowerup('Incremental', powerupCode, 'Mo\'s Incremental Everything Powerup', {
    slots: [
      {
  ...
  const unregisterQueueCSS = async (plugin: RNPlugin) => {
    // CSS cleanup function - no longer needed after removing problematic CSS
  };
...
.dark .rn-queue__card-counter:after {
  ...
  plugin.app.registerCommand({
    ...
    name: 'Mo\'s Incremental Everything',
    action: async () => {
      createExtract();
    },
  });
  ...
  plugin.app.registerCommand({
    ...
    name: 'Untag Mo\'s Incremental Everything',
    action: async () => {
      const selection = await plugin.editor.getSelection();
      if (!selection) {
        return;
  ...
  plugin.app.registerCommand({
    ...
    name: 'Debug Mo\'s Incremental Everything',
    action: async () => {
      const rem = await plugin.focus.getFocusedRem();
      if (!rem) {
        return;
